"""
GeoServer Generic provider for PyGeoFetch.

Generic OGC WMS/WFS/WCS GeoServer endpoint. Configure URL and auth via settings.
"""
...
class GeoserverGenericProvider(AbstractBaseProvider):
    ...
    DISPLAY_NAME = "GeoServer Generic"
    REQUIRES_AUTH = False
    DESCRIPTION = "Generic OGC WMS/WFS/WCS GeoServer endpoint. Configure URL and auth via settings."
    SATELLITES = []
    BASE_URL = ""
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://geoserver.org/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            endpoint_url=self.BASE_URL, docs_url="https://geoserver.org/",
