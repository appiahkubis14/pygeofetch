"""
ISRO Bhuvan provider for PyGeoFetch.

Indian satellite data: ResourceSat, Cartosat, Oceansat. Open access.
"""
...
class IsroBhuvanProvider(AbstractBaseProvider):
    ...
    DISPLAY_NAME = "ISRO Bhuvan"
    REQUIRES_AUTH = False
    DESCRIPTION = "Indian satellite data: ResourceSat, Cartosat, Oceansat. Open access."
    SATELLITES = ['ResourceSat-2', 'ResourceSat-2A', 'Cartosat-2', 'Oceansat-2']
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://bhuvan.nrsc.gov.in/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=0.65, resolution_max_m=500.0,
            endpoint_url=self.BASE_URL, docs_url="https://bhuvan.nrsc.gov.in/",
