...
class AirbusOneatlasProvider(AbstractBaseProvider):
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://oneatlas.airbus.com/api-docs/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=0.5, resolution_max_m=6.0,
            endpoint_url=self.BASE_URL, docs_url="https://oneatlas.airbus.com/api-docs/",
