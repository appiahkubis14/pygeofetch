...
class OpentopographyProvider(AbstractBaseProvider):
    ...
    DEM_TYPES = {
        "srtm30": "SRTMGL30", "srtm90": "SRTMGL3", "srtm1arc": "SRTMGL1",
        "cop30": "COP30", "cop90": "COP90", "nasadem": "NASADEM",
        "alos": "AW3D30",
    }
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if not api_key:
            raise AuthenticationError("OpenTopography requires an API key from portal.opentopography.org")
        from datetime import datetime, timezone, timedelta
        session = AuthSession(provider=self.PROVIDER_ID, access_token=api_key,
    ...
    def search(self, query: SearchQuery) -> List[SatelliteData]:
        ...
        for dem_key, dem_type in self.DEM_TYPES.items():
            ...
            item = SatelliteData(
                ...
                assets={"dem": SatelliteAsset(key="dem", href=asset_url, roles=["data"], media_type="image/tiff")},
    ...
    def download(self, data: SatelliteData, destination: Path, options: DownloadOptions) -> DownloadResult:
        ...
        for key, asset in (data.data_assets or data.assets).items():
            ...
            try:
                with httpx.stream("GET", href, timeout=options.timeout_seconds, follow_redirects=True) as resp:
        ...
        if not output_paths:
            return DownloadResult(status=DownloadStatus.FAILED, data_id=data.id, provider=self.PROVIDER_ID, error="Download failed")
        return DownloadResult(status=DownloadStatus.COMPLETED, data_id=data.id, provider=self.PROVIDER_ID,
                              ...
                              bytes_downloaded=total_bytes, duration_seconds=time.time() - start)

    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            provider_id=self.PROVIDER_ID, name=self.DISPLAY_NAME, description=self.DESCRIPTION,
            auth_type="api_key", satellites=["SRTM", "Copernicus", "ALOS"],
            search=True, download=True, supports_aoi_filter=True, supports_date_filter=False,
            requires_auth=True, has_quota=True, regions=["global"],
            resolution_min_m=30.0, resolution_max_m=1000.0,
            endpoint_url=self.BASE_URL, docs_url="https://opentopography.org/developers",
