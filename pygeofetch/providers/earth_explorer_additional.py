...
class EarthExplorerAdditionalProvider(AbstractBaseProvider):
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://earthexplorer.usgs.gov/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=1.0, resolution_max_m=80.0,
            endpoint_url=self.BASE_URL, docs_url="https://earthexplorer.usgs.gov/",
