...
class FederatedSearcher:
    ...
    def search(
        ...
        logger.info(f"Searching {len(providers)} provider(s): {', '.join(providers)}")
        ...
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(providers))) as executor:
            ...
            for future in as_completed(futures, timeout=self.timeout_per_provider * 2):
                ...
                try:
                    ...
                    logger.info(f"  {provider_id}: {len(results)} results")
                    all_results.extend(results)
                except TimeoutError:
                    logger.warning(f"  {provider_id}: search timed out")
                    provider_errors[provider_id] = "Search timed out"
                except Exception as exc:
                    logger.warning(f"  {provider_id}: search failed: {exc}")
        ...
        logger.info(f"Search complete: {len(final)} unique results from {len(providers)} providers")

        if provider_errors:
            logger.warning(f"Providers with errors: {provider_errors}")
    ...
    def _get_provider(self, provider_id: str) -> Any:
        """Get or create a provider instance with session from auth manager."""
        from pygeofetch.providers import get_provider
        prov = get_provider(provider_id)

        if self.auth_manager and prov.REQUIRES_AUTH:
            # Always re-attempt auth — do not cache unauthenticated providers
            try:
                session = self.auth_manager.authenticate(provider_id)
                prov.set_session(session)
                logger.debug(f"Authenticated {provider_id} for search")
            except Exception as exc:
                logger.warning(f"{provider_id}: authentication failed: {exc}")
                # Leave provider unauthenticated — require_auth() in search() will raise
