...
from pygeofetch.models.satellite_data import (
    DataFormat, ProviderCapabilities, QuotaInfo,
...
from pygeofetch.providers.base import (
    AbstractBaseProvider, AuthenticationError, SearchError,
)
...
class TerraboticsProvider(AbstractBaseProvider):
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if not api_key:
            raise AuthenticationError(
                "TerraBotics requires an API key. "
                "Contact support at: https://terrabotics.earth"
            )
        ...
        try:
            ...
            if resp.status_code == 401:
                raise AuthenticationError("Invalid TerraBotics API key.")
    ...
    def search(self, query: SearchQuery) -> List[SatelliteData]:
        ...
        try:
            ...
            raise SearchError(f"TerraBotics search failed: {exc}") from exc
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if download_url:
                    ...
                    with httpx.stream("GET", download_url, headers={"X-API-Key": api_key},
                                      timeout=options.timeout_seconds) as dl:
                        self._handle_http_error(dl)
                        with open(out_file, "wb") as f:
                            for chunk in dl.iter_bytes(chunk_size=int(options.chunk_size_mb * 1024 * 1024)):
                                f.write(chunk)
