...
@click.option("--format", "-f", default="geojson",
              type=click.Choice(["geojson","gpkg","shp"]))
...
def vectorize_cmd(input, output, band, threshold, format, min_area):
    ...
    e = _engine()
    r = e.post.vectorize(input, output=output, band=band, threshold=threshold,
                         format=format, min_area=min_area)
    _pr(r, "vectorize")
...
@click.option("--method", "-m", default="simplify",
              type=click.Choice(["simplify","buffer"]))
@click.option("--output", "-o", default=None)
def smooth_cmd(input, tolerance, method, output):
    """Smooth/simplify vector geometries (Douglas-Peucker or buffer-unbuffer)."""
    e = _engine(); r = e.post.smooth(input, tolerance=tolerance, method=method, output=output)
...
def regularize_cmd(input, output):
    ...
    e = _engine(); r = e.post.regularize(input, output=output); _pr(r, "regularize")
...
@click.option("--stats", default="count,mean,median,min,max,std",
...
def buffer_cmd(input, distance, cap_style, output):
    ...
    e = _engine(); r = e.post.buffer(input, distance=distance, cap_style=cap_style, output=output)
...
def centroids_cmd(input, output):
    ...
    e = _engine(); r = e.post.centroids(input, output=output); _pr(r, "centroids")
...
def metrics_cmd(input, output):
    ...
    e = _engine(); r = e.post.add_geometry_metrics(input, output=output); _pr(r, "geometry-metrics")


@post.command("compress")
@click.argument("input", type=click.Path(exists=True))
@click.option("--method", "-m", default="lzw", show_default=True,
              type=click.Choice(["lzw","deflate","zstd","packbits"]))
@click.option("--output", "-o", default=None)
def compress_cmd(input, method, output):
    """Apply lossless compression to a GeoTIFF."""
    e = _engine(); r = e.post.compress(input, method=method, output=output)
    _pr(r, f"compress ({method})")


@post.command("cog")
@click.argument("input", type=click.Path(exists=True))
@click.option("--compress", "-c", default="deflate",
