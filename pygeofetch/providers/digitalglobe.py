"""
DigitalGlobe Open Data provider for PyGeoFetch.

DigitalGlobe disaster response open data. Free for humanitarian use.
"""
...
from pygeofetch.models.satellite_data import DataFormat, ProviderCapabilities, QuotaInfo, SatelliteData, SatelliteAsset
...
class DigitalglobeProvider(AbstractBaseProvider):
    ...
    DISPLAY_NAME = "DigitalGlobe Open Data"
    REQUIRES_AUTH = False
    DESCRIPTION = "DigitalGlobe disaster response open data. Free for humanitarian use."
    SATELLITES = ['WorldView-2', 'WorldView-3', 'GeoEye-1']
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://www.maxar.com/open-data"
            )
        session = AuthSession(
            provider=self.PROVIDER_ID,
            access_token=token or credentials.username or "anonymous",
    ...
    def search(self, query: SearchQuery) -> List[SatelliteData]:
        ...
        if self._session and self._session.access_token and self._session.access_token not in ("anonymous", ""):
        ...
        try:
            resp = httpx.get(f"{self.BASE_URL}/search", params=params, headers=headers,
    ...
    def _parse_item(self, item: Dict[str, Any]) -> SatelliteData:
        ...
        if isinstance(raw, (list, tuple)) and len(raw) == 4:
            bbox = tuple(float(x) for x in raw)
        cloud_raw = item.get("cloud_cover") or item.get("cloudCover") or (item.get("properties") or {}).get("eo:cloud_cover")
        return SatelliteData(
            id=item_id, provider=self.PROVIDER_ID,
    ...
    def download(self, data: SatelliteData, destination: Path, options: DownloadOptions) -> DownloadResult:
        ...
        for key, asset in (data.data_assets or data.assets).items():
            ...
            try:
                with httpx.stream("GET", asset.href, headers=headers,
        ...
        if not output_paths:
            return DownloadResult(status=DownloadStatus.FAILED, data_id=data.id,
                                  provider=self.PROVIDER_ID, error="No assets downloaded")
        return DownloadResult(status=DownloadStatus.COMPLETED, data_id=data.id,
                              provider=self.PROVIDER_ID, output_path=output_paths[0],
                              output_paths=output_paths, bytes_downloaded=total_bytes,
                              duration_seconds=time.time() - start)

    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            provider_id=self.PROVIDER_ID, name=self.DISPLAY_NAME, description=self.DESCRIPTION,
            auth_type="none",
            satellites=['WorldView-2', 'WorldView-3', 'GeoEye-1'],
            search=True, download=True,
            supports_sar=False, supports_sub_meter=True,
            supports_aoi_filter=True, supports_cloud_filter=True, supports_date_filter=True,
            requires_auth=self.REQUIRES_AUTH, has_quota=self.REQUIRES_AUTH,
            regions=["global"],
            resolution_min_m=0.3, resolution_max_m=2.0,
            endpoint_url=self.BASE_URL, docs_url="https://www.maxar.com/open-data",
