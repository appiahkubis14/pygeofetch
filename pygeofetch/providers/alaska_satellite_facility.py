"""
Alaska Satellite Facility provider for PyGeoFetch.
...
class AlaskaSatelliteFacilityProvider(AbstractBaseProvider):
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://asf.alaska.edu/api/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=5.0, resolution_max_m=100.0,
            endpoint_url=self.BASE_URL, docs_url="https://asf.alaska.edu/api/",
