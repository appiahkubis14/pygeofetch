...
class NoaaBigDataProvider(AbstractBaseProvider):
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://registry.opendata.aws/noaa-goes/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=500.0, resolution_max_m=10000.0,
            endpoint_url=self.BASE_URL, docs_url="https://registry.opendata.aws/noaa-goes/",
