...
def _print_result(result, operation: str) -> None:
    if result.success:
        ...
        console.print(f"[green]✓[/] {operation} → {result.output_path} ({size_mb:.1f} MB, {result.duration_seconds:.2f}s)")
    else:
...
def clip_cmd(input, bbox, geometry, output, all_touched):
    ...
    r = e.preprocess.clip(input, bbox=bbox_tuple, geometry=geometry, output=output, all_touched=all_touched)
...
@click.option("--resampling", default="bilinear", show_default=True,
...
def reproject_cmd(input, crs, resampling, resolution, output):
    ...
    e = _engine()
    r = e.preprocess.reproject(input, crs=crs, resampling=resampling, resolution=resolution, output=output)
    _print_result(r, f"reproject → {crs}")


@preprocess.command("resample")
@click.argument("input", type=click.Path(exists=True))
@click.option("--resolution", "-r", default=None, type=float, help="Target resolution in CRS units.")
@click.option("--scale", "-s", default=None, type=float, help="Scale factor (0.5=half, 2.0=double).")
@click.option("--method", "-m", default="bilinear", show_default=True,
              type=click.Choice(["nearest","bilinear","cubic","lanczos","average"]))
@click.option("--output", "-o", default=None)
def resample_cmd(input, resolution, scale, method, output):
    ...
    if not resolution and not scale:
        console.print("[red]Provide --resolution or --scale[/]"); sys.exit(1)
    e = _engine()
    r = e.preprocess.resample(input, resolution=resolution, scale_factor=scale, method=method, output=output)
    _print_result(r, "resample")


@preprocess.command("cloud-mask")
@click.argument("input", type=click.Path(exists=True))
@click.option("--method", "-m", default="scl", show_default=True,
...
@click.option("--method", "-m", default="interpolate",
...
def cloud_fill_cmd(input, time_series, method, output):
    ...
    if not time_series:
        console.print("[red]Provide at least one time series scene[/]"); sys.exit(1)
    e = _engine()
...
@click.argument("input", type=click.Path(exists=True))
@click.option("--method", "-m", default="dos1", show_default=True,
...
@click.option("--method", "-m", default="cosine",
...
@click.option("--method", "-m", default="brovey",
...
@click.argument("inputs", nargs=-1, type=click.Path(exists=True))
@click.option("--method", "-m", default="median",
...
def tile_cmd(input, tile_size, overlap, output_dir, min_coverage):
    ...
    e = _engine()
    r = e.preprocess.tile(input, tile_size=tile_size, overlap=overlap,
