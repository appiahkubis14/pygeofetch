...
def _make_geotiff(path: Path, width=10, height=10, bands=1,
                  crs_epsg=4326, identity_transform=False) -> Path:
    """
    ...
    import numpy as np
    profile = dict(
        driver="GTiff", dtype="uint16", count=bands,
        width=width, height=height, crs=crs, transform=transform,
    )
    with rasterio.open(path, "w", **profile) as ds:
        for b in range(1, bands + 1):
            ds.write((np.random.randint(1, 1000, (height, width),
                      dtype=np.uint16)), b)
    return path
...
class TestBug1CredentialRoundtrip:
    ...
    def test_add_credentials_oauth2(self):
        ...
        from pygeofetch import PyGeoFetch
        engine = PyGeoFetch(log_level="WARNING")
        engine.add_credentials("sentinel_hub", client_id="cid-abc",
    ...
    def test_count_contract(self, tmp_path):
        ...
        def fake_download(data, dest, opts):
            ...
            return DownloadResult(status=DownloadStatus.COMPLETED, data_id=data.id,
                                  provider=data.provider, output_path=p,
                                  bytes_downloaded=2)
...
class TestCapability2S1Constellation:
    ...
    def test_warn_if_outdated_constellation_fires(self, caplog):
        ...
        if date.today() >= date(2026, 7, 1):
            ...
            assert any("S1A" in m or "decommission" in m.lower() or "S1C" in m
...
class TestCapability3OrbitFiles:
    ...
    def test_find_matching_orbit_file(self):
        ...
        # Simulate HTML directory listing
        listing_html = '''
            <a href="S1C_OPER_AUX_POEORB_OPOD_20260520T121000_V20260501T000000_20260503T000000.EOF">
            <a href="S1C_OPER_AUX_POEORB_OPOD_20260622T121000_V20260601T000000_20260603T000000.EOF">
            <a href="S1C_OPER_AUX_POEORB_OPOD_20260705T121000_V20260701T000000_20260703T000000.EOF">
        '''
        acq_dt = datetime(2026, 6, 2, 5, 30, 0)
...
class TestLogging:
    ...
    def test_render_progress_bar_full(self):
        ...
        from pygeofetch.core.logging import _render_progress_bar
        result = _render_progress_bar(
            completed=8, total=8, filename="file.zip",
            bytes_done=1_000_000_000, bytes_total=1_000_000_000, speed_bps=1_000_000,
        )
    ...
    def test_info_search_log_format(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="pygeofetch"):
            ...
            lg.info("Search complete: %d scene(s) across %d provider(s) in %.1fs",
                    14, 2, 3.1)
    ...
    def test_info_download_log_format(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="pygeofetch"):
            ...
            lg.info("  \u2713 %-45s %6.0f MB  %5.1fs",
                    "S1C_IW_GRDH_1SDV_20260601T053000"[:45], 2100, 17.8)
            lg.info("  post-processing: %s", "reproject:EPSG:32630 \u2192 cog")
            lg.info("Download complete: %d/%d succeeded  %.0f MB total",
                    2, 2, 3900)
...
class TestFinalEngineSmoke:
    ...
    def test_engine_pipeline_all_41_methods(self):
        ...
        pl = (
            ...
            # Spectral indices (17)
            .ndvi().evi().savi().ndwi().mndwi().ndbi().ndsi().ndmi()
            .nbr().dnbr().tct().pca().texture().lst().albedo().band_math().stack()
            # Post-processing (9)
            .vectorize().smooth().regularize().zonal_stats().buffer()
            .centroids().add_geometry_metrics().compress().cog()
            # SAR (4)
            .despeckle().calibrate().flood_map().coherence()
        )
        assert len(pl._steps) == 41, (
            f"Expected 41 pipeline steps, got {len(pl._steps)}"
        )
