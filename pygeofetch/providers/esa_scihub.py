...
class EsaScihubProvider(AbstractBaseProvider):
    ...
    REQUIRES_AUTH = False
    DESCRIPTION = "Mirror access to Copernicus Sentinel data. No login for open mirrors."
    ...
    def authenticate(self, credentials: Credentials) -> AuthSession:
        ...
        if self.REQUIRES_AUTH and not token and not credentials.username:
            raise AuthenticationError(
                f"{self.DISPLAY_NAME} requires credentials. See: https://scihub.copernicus.eu/"
            )
        session = AuthSession(
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        return ProviderCapabilities(
            ...
            regions=["global"],
            resolution_min_m=5.0, resolution_max_m=300.0,
            endpoint_url=self.BASE_URL, docs_url="https://scihub.copernicus.eu/",
